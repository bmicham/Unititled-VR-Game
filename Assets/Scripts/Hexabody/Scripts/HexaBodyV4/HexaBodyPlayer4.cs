...
namespace HexabodyVR.PlayerController
{
    public class HexaBodyPlayer4 : MonoBehaviour
    {
        ...
        protected virtual void UpdateMaxCrouch()
        {
            ...
            if (_airCrouch && ratio > StopAirCrouch || KP_Inputs.input.CrouchRate < .2f)
            {
                _airCrouch = false;
            }
            else if (!_airCrouch)
        ...
        protected virtual void UpdateCrouchAmount()
        {
            if (IsJumpCycle)
            {
                if (KP_Inputs.input.CrouchState.JustActivated)
                {
                    StopJumpCycle();
                }
                else if (KP_Inputs.input.JumpState.JustActivated)
                {
            ...
            if (KP_Inputs.input.JumpState.Active)
            {
            ...
            else if (KP_Inputs.input.CrouchState.Active && (IsStanding || (IsJumpCycle && !IsJumping)))
            {
                float goal;
                float sign = 1f;
                float factor = 1f;

                if (KP_Inputs.input.CrouchRate < 0f) //standing up
                {
                    goal = -TipToesOffset;
                    sign = -1f;
                    if (CrouchSpeed > 0)
                ...
                CrouchAmount = Mathf.MoveTowards(CrouchAmount, goal, KP_Inputs.input.CrouchRate * Time.deltaTime * CrouchSpeed * factor);
            }
            else if (IsJumpCycle)
            {
        ...
        {
            if (!KP_Inputs.input.JumpState.Active && !KP_Inputs.input.CrouchState.Active)
            {
                var limit = Sprinting ? RunningCrouchLevel : WalkingCrouchLevel;
                var target = Mathf.Lerp(MaxCrouch, GetCrouchLevel(limit), KP_Inputs.input.Secondary2AxisInputAxes.magnitude);
            ...
            if (KP_Inputs.input.JumpState.JustActivated && JumpStage != JumpStage.Stomping)
            {
                CrouchTarget = GetCrouchTarget(CrouchAmount);
                crouchLimit = CrouchTarget;
            }

            if (!IsJumpCycle && !KP_Inputs.input.AButtonPressed) // && IsGrounded || !IsGrounded && Inputs.CrouchState.JustDeactivated)
            {
                if (CrouchLevels.Length <= 0) return;

                if (KP_Inputs.input.Secondary2AxisInputAxes.magnitude < .05f)
                {
                    CrouchTarget = GetCrouchTarget(CrouchAmount);
                }
                else
        ...
        {
            if (KP_Inputs.input.BButtonDown)
            {
                Calibrate();
            }
            ...
            if (KP_Inputs.input.JumpState.JustDeactivated && (_legStatus == LegStatus.Standing || JumpStage == JumpStage.LandingGrounded || IsGrounded))
            {
                _jump = true;
                JumpStage = JumpStage.Jumping;
                _legStatus = LegStatus.Jumping;
        ...
        {
            var movement = KP_Inputs.input.Secondary2AxisInputAxes;
        ...
        protected virtual void CheckSprinting()
        {
            if (SprintRequiresDoubleClick)
            {
                ...
                if (!Sprinting && KP_Inputs.input.Secondary2AxisInputPressed)
                {
            ...
            else
            {
                ...
                if (Sprinting && KP_Inputs.input.Secondary2AxisInputPressed)
                    Sprinting = false;
                else if (!Sprinting && KP_Inputs.input.Secondary2AxisInputPressed)
                    Sprinting = true;
            }

            if (KP_Inputs.input.Secondary2AxisInputAxes.magnitude < .01f)
            {
                Sprinting = false;
            }
        }
